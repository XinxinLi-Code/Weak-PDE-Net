"""Derivative utilities developed with reference to Weak-PDE-LEARN.

Original project: https://github.com/punkduckable/Weak_PDE_LEARN
"""

from typing import List, Dict;
import numpy;
import torch;


class Derivative():
    """
    Encoding: A 1D NumPy integer array representing a partial derivative operator.
    For `n` spatial variables, the array should contain `n + 1` elements.
    For example, `[1, 0, 0, 0]` denotes a first-order derivative with respect to `t` and zero-order derivatives with respect to `x`, `y`, and `z`.
    The 0th element indicates the order of the time derivative, while the `i`-th element (`i > 0`) indicates the order of the derivative with respect to the `(i-1)`-th spatial variable.
    Currently, only one, two, or three spatial variables are supported.

    Order: The sum of all elements in `Encoding`. It represents the total number of partial derivatives applied to a function.
    This value is required when calculating the integral of the product of the weight function and the library term (where integration by parts is applied for each partial derivative in the operator).
    """

    def __init__(self,
                 Encoding: numpy.ndarray) -> None:
        """ 
        Initializer.

        -----------------------------------------------------------------------
        Arguments:

        Encoding: See the class docstring.
        """

        # First, cast to integer array. This also returns a copy of Encoding.
        Encoding: numpy.ndarray = Encoding.astype(dtype=numpy.int32);

        # Check that Encoding is a 1D array.
        assert (len(Encoding.shape) == 1);

        # Determine input dimension. Currently, we only support n = 2, 3, 4.
        Input_Dim: int = Encoding.size;
        assert (Input_Dim == 2 or Input_Dim == 3 or Input_Dim == 4);

        # Calculate the order (total number of partial derivatives) of this
        # derivative operator.
        self.Order: int = numpy.sum(Encoding).item();

        # Check that each element of encoding is a non-negative integer.
        for i in range(Encoding.size):
            assert (Encoding[i] >= 0);

        # Assuming the input passes all checks, assign it.
        self.Encoding = Encoding;

    def to(self, device: torch.device):
        self.Encoding = self.Encoding.to(device)
        return self

    def __str__(self) -> str:
        """ 
        This function returns a string that contains a human-readable
        expression for the derivative operator that this object represents. It
        is mainly used for printing. 
        """

        Buffer: str = "";

        # Time derivative.
        if (self.Encoding[0] == 1):
            Buffer += "D_t ";
        elif (self.Encoding[0] > 1):
            Buffer += ("D_t^%u " % self.Encoding[0]);

        # x derivative.
        if (self.Encoding[1] == 1):
            Buffer += "D_x ";
        elif (self.Encoding[1] > 1):
            Buffer += ("D_x^%u " % self.Encoding[1]);

        # y derivative (if it exists).
        Num_Spatial_Vars: int = self.Encoding.size - 1;
        if (Num_Spatial_Vars > 1):
            if (self.Encoding[2] == 1):
                Buffer += "D_y ";
            elif (self.Encoding[2] > 1):
                Buffer += ("D_y^%u " % self.Encoding[2]);

        # z derivative (if it exists).
        if (Num_Spatial_Vars > 2):
            if (self.Encoding[3] == 1):
                Buffer += "D_z ";
            elif (self.Encoding[3] > 1):
                Buffer += ("D_z^%u " % self.Encoding[3]);

        return Buffer;

    def Is_Child_Of(self, D) -> bool:
        """ 
        This function determines whether self is a "child" of the derivative
        operator D. What does this mean? Let D_1 and D_2 be derivative objects.
        Let [p_1, ... , p_n] and [q_1, ... , q_m] denote D_1's and D_2's
        encoding vectors, respectively. Let l = min{n, m}. D_1 is a child of
        D_2 if and only if p_k <= q_k for k \in {1, 2, ... , l} and p_k = 0
        for k > l. This is equivalent to saying that there is a derivative operator
        D_3 such that D_2 U = D_3 D_1 U.

        -----------------------------------------------------------------------
        Arguments:

        D : A derivative operator. This function determines whether self is a child
        of D.

        -----------------------------------------------------------------------
        Returns:

        A boolean; True if self is a child of D, False otherwise. 
        """

        # First, find l, the minimum length of self.Encoding and D.Encoding.
        n: int = len(self.Encoding);
        m: int = len(D.Encoding);
        l: int = min(n, m);

        # Check that self.Encoding[k] <= D.Encoding[k] for k <= l.
        for k in range(l):
            if (self.Encoding[k] > D.Encoding[k]):
                return False;

        # Assuming self.Encoding is longer than D.Encoding, make sure that the
        # components of self.Encoding after index l are zero.
        for k in range(l + 1, n):
            if (self.Encoding[k] != 0):
                return False;

        # If we make it here, then self is a child of D.
        return True;

    def Get_State(self) -> Dict:
        """
        This function helps serialize self. It returns a dictionary that can be
        used to create self from scratch. You can recover a copy of self by 
        passing this dictionary to the Build_Derivative_From_State function.
        
        -----------------------------------------------------------------------
        Returns:

        A dictionary with one key: "Encoding".
        """

        # We can start things off with the Powers attribute.
        State: Dict = {"Encoding": self.Encoding};

        return State;


def Build_Derivative_From_State(State: Dict) -> Derivative:
    """
    This function builds a new Derivative object from a State dictionary. It 
    then returns that object. 

    ---------------------------------------------------------------------------
    Arguments:

    State: A dictionary. This should either be the dictionary returned by the 
    Derivative class's Get_State method, or an unpickled copy of one. 

    ---------------------------------------------------------------------------
    Returns:

    A new derivative object.
    """

    return Derivative(State["Encoding"]);
