...
for path in (PROJECT_ROOT, SG_DIR):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from sg_params import Params
from PDE_Discover import PDE_Discover


def set_all_seeds(seed=420):
    random.seed(seed)
    np.random.seed(seed)
...
def load_sg_data(data_path=None, time_steps=70):
    """Load the Sine-Gordon dataset and return coordinates and field values."""
    if data_path is None:
        data_path = PROJECT_ROOT / "datasets" / "Sine_Gordon.mat"
    mat_data = scipy.io.loadmat(data_path)

    t = mat_data["t"].flatten()[:time_steps]
    x = mat_data["x"].flatten()
    y = mat_data["y"].flatten()
    field = np.transpose(mat_data["U_exact"][0][0], (2, 1, 0))[:time_steps]

    # The coordinate order used by PDE_Discover is (t, x, y).
    t_grid, x_grid, y_grid = np.meshgrid(t, x, y, indexing="ij")
    coordinates = np.column_stack(
        (t_grid.ravel(), x_grid.ravel(), y_grid.ravel())
    )
    values = np.real(field).reshape(-1, 1)
    input_data = torch.tensor(
        np.column_stack((coordinates, values)), dtype=torch.float32
    )
    return coordinates, values, input_data, field.shape


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Discover the Sine-Gordon equation with configurable sampling and noise."
    )
    parser.add_argument(
        "--sample_ratio",
        type=float,
    ...
    config = Params()
    seed = int(getattr(config, "seed", 420))
    set_all_seeds(seed)
    config.sigma_NR = sigma_nr

    coordinates, values, input_data, grid_shape = load_sg_data()
    ...
    sampling_idx = get_incremental_sampling_idx(
        total_points,
        sample_ratio,
        save_dir=str(SG_DIR / "sampling_idx"),
        seed=seed,
    ...
    save_config(config, save_dir=str(SG_DIR / "configs"))
